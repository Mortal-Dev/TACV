...
public partial struct VehicleRequestSystem : ISystem
{
    ...
    public static void ServerRecieveClientRequestVehicleEnter(ushort clientId, Message message)
    {
        ...
        void SendConfirmVehicleMessages()
        {
            ...
            if (!vehicleSeatComponent.hasOwnershipCapability) return;

            Message changeVehicleOwnershipMessage = Message.Create(MessageSendMode.Reliable, NetworkMessageId.ServerChangeEntityOwnership);

            message.Add(entityManager.GetComponentData<NetworkedEntityComponent>(vehicleEntity).networkEntityId);
